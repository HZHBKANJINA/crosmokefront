<template>
  ...
  <table>
      <thead>
        ...
        <tbody v-if="odabraniClan">
          <template v-for="anketa in filtriraneAnkete" :key="anketa._id">
            <tr>
              <td>1</td>
              <td>BROJ:</td>
              <td>{{ anketa.broj }}</td>
...
export default {
  ...
  mounted(){
    ...
    Promise.all([this.dobaviClana(), this.dobaviAnketu()]).then(() => {
    console.log('Clanovi:', this.clanovi);
    console.log('Ankete:', this.ankete);
    
    // Check data integrity
    this.clanovi.forEach(clan => {
      const matchingAnketa = this.ankete.find(anketa => anketa._id === clan.spremi_anketu);
      if (!matchingAnketa) {
        console.warn(`No matching anketa found for clan ${clan.broj_clana} (ID: ${clan._id})`);
      }
    });
  });
  },
  watch: {
  odabraniClan(newVal) {
    console.log('Odabrani član promijenjen:', newVal);
    console.log('Filtrirane ankete:', this.filtriraneAnkete);
  }
}
